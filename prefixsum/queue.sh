#!/bin/bash
if [ ! -e .passed_prefixsum ]; then
    echo 'Must pass `make test` before queueing jobs'
    exit 1
fi
running=$(squeue -u "$USER" | egrep "bench_pr.*[R|PD]")
if [ -n "$running" ]; then
    echo "------------------------------------"
...
sbatch --partition=Centaurus --time=02:00:00 --nodes=1 --ntasks-per-node=16 bench_prefixsum.sh
echo "-----------------------"
echo
